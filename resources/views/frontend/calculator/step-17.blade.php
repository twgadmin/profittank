...
<section class="sevanten revenue_slides row align-items-center">
    <div class="col-12 col-lg-8 mx-auto">
        <div class="threechexk">
            <h3 class="mb-5">Are you workers compensation premiums over $25K per year?
            </h3>
            <div class="custom-choose mb-5 mt-5">
                <input type="radio" class='radio_next' id="workers-1" value="Yes" name="company_premium"
                    {{session()->get('company_premium') == 'Yes' ? 'checked' : ''}}>
                <label for="workers-1">
                    Yes
                </label>
                <input type="radio" class='radio_next' id="workers-2" value="No" name="company_premium"
                    {{session()->get('company_premium') == 'No' ? 'checked' : ''}}>
                <label for="workers-2">
                    No
                </label>
                <input type="radio" class='radio_next' id="workers-3" value="Unknown" name="company_premium"
                    {{session()->get('company_premium') == 'Unknown' ? 'checked' : ''}}>
                <label for="workers-3">
                    Unknown
                </label>
            </div>
            <div class="form__field form__field_select_choose">
                <input type="hidden" id="option_value"
                    value="{{session()->get('company_premium')}}" data-validation="custom required"
