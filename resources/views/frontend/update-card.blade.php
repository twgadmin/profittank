...
<section class="subSec hding-3 pad-50">
   <div class="container">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">Current Card Details</div>
        <div class="panel-body">
            <div class="row">
                <div class="col-md-6 offset-md-3">
                    <div class="row text-center">
                        @if (isset($data) && isset($data['token']) && isset($data['token']['card']))
                        <div class="col-md-6 col-lg-6 col-xs-6">
                            <p><label>Card Type: </label> {!! $data['token']['card']['brand']  !!}</p>
                        </div>
                        <div class="col-md-6 col-lg-6 col-xs-6">
                            <p><label>Card No: </label>**** **** **** {!! $data['token']['card']['last4'] !!}</p>
                        </div>
                        <div class="col-md-6 col-lg-6 col-xs-6">
                            <p><label>Expiry: </label> {!! $data['token']['card']['exp_month'] . '/' . $data['token']['card']['exp_year']  !!}</p>
                        </div>
                        @endif
                    </div>
                </div>
            </div>
            <form id="subscribe-plan" method="POST" role="form" action="{!! route('update-card-details') !!}">
                    @csrf
                    @method('POST')
                <div class="row text-center ">
